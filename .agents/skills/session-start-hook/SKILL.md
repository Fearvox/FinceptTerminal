...
description: Creating and developing startup hooks for Codex on the web. Use when the user wants to set up a repository for Codex on the web, create a SessionStart hook to ensure their project can run tests and linters during web sessions.
---

# Startup Hook Skill for Codex on the web

Create SessionStart hooks that install dependencies so tests and linters work in Codex on the web sessions.
...
- `$CLAUDE_CODE_REMOTE` - If running in a remote environment (i.e. Codex on the web)
...
```bash
mkdir -p .Codex/hooks
cat > .Codex/hooks/session-start.sh << 'EOF'
#!/bin/bash
set -euo pipefail
...
Add to `.Codex/settings.json` (create if doesn't exist):
```json
{
  "hooks": {
    "SessionStart": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "$CLAUDE_PROJECT_DIR/.Codex/hooks/session-start.sh"
          }
        ]
      }
    ]
...
If `.Codex/settings.json` exists, merge the hooks configuration.
...
```bash
CLAUDE_CODE_REMOTE=true ./.Codex/hooks/session-start.sh
```
...
* Hook execution mode: Syncronous
  * inform user that hook is running syncronous and the below trade-offs. Let them know that we can change it to async if they prefer faster session startup.
    * Pros: Guarantees dependencies are installed before your session starts, preventing race conditions where Codex might try to run tests or linters before they're ready
