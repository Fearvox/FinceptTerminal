...
- Codex-only 2x2 matrix runs (those use `blind.sh`)
...
2. Commit benchmark + docs + src separately from local config (`.Codex/`, `.worktrees/`, `.playwright-mcp/`)
...
| Grok (R006) | 71 | 1 | 83% | 4+ | competitive | 3 Codex Sonnet | B- |

## Integration

- **After benchmark:** use `benchmark-ingest` skill
- **For Codex 2x2 matrix:** use `blind.sh` (env isolation)
