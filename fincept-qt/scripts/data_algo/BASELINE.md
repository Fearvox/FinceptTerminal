...
# Check 6 — scanner liveness (hits real endpoints)
# Requires keychain entries (manifold-api) for full coverage
python3 _smoke_scanners.py
# expect (with keychain configured): 1/1 OK as of iter-4/5
# expect (without keychain):         1/1 SKIPPED with helpful reason
```
